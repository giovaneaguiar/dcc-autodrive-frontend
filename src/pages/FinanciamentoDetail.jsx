...
                if (id) {
                    // Se for uma edição, busca os dados do financiamento
                    const financiamentoResponse = await financiamentoService.getById(id);
                    const financiamentoData = financiamentoResponse.data;
                    
                    // Mapeia o idVenda retornado pela API para o objeto completo de venda
                    const vendaObj = allVendas.find(v => v.id === financiamentoData.idVenda) || null;
...
        setLoading(true);
        try {
            const financiamentoToSave = { ...financiamento };

            // Antes de salvar, transforma o objeto de venda de volta para o formato de ID
            if (financiamentoToSave.venda) {
                financiamentoToSave.venda = { id: financiamentoToSave.venda.id };
            }

            if (id) {
                await financiamentoService.update(id, financiamentoToSave);
                setModalMessage("Financiamento atualizado com sucesso!");
            } else {
                await financiamentoService.create(financiamentoToSave);
